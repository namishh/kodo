...
M.open = function(buf)
  if vim.fn.expand "%" == "" or buf then
    ...
    for _, winnr in ipairs(api.nvim_list_wins()) do
      ...
      if api.nvim_buf_is_valid(bufnr) and (vim.bo[bufnr]).ft ~= "dashboard" then
    ...
    local dash = api.nvim_create_namespace "dasboard"
    local horiz_pad_index = math.floor((api.nvim_win_get_width(0) / 2) - (36 / 2)) - 2

    for i = abc, abc + #header - 2 do
      api.nvim_buf_add_highlight(buf, dash, "dasboardart", i, horiz_pad_index, -1)
    end

    for i = abc + #header - 2, abc + #dashboard do
      api.nvim_buf_add_highlight(buf, dash, "dashboardbutton", i, horiz_pad_index, -1)
    end
